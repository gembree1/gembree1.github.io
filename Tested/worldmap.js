// ALL code for worldmap graph; includes functions.js and old index code

function worldmap_main() {
    ...
    // slider
    d3.select("#map_container").insert("p", ":first-child").append("input")
        .attr("type", "range")
        .attr("min", "1970")
        .attr("max", "2016")
    ...
    d3.select("#map_container").insert("h2", ":first-child").attr("id","map_h2").text(headline + init_year);

    // init map container, projection
    var width = 1360,
        height = 425; //orig 960
    var svg_map = d3.select("#map_container").insert("svg")
        .attr("id", "map")
        .attr("height", height)
        .attr("width", width);
    ...
    var margin = {
        ...
        bottom: 50,
        left: 80
    }; //{top: 50, right:10, bottom:50, left:30};
    ...
    //  var svg_bars = d3.select("#map_container")
    //      .append("svg")
    //        .attr("id", "bars")
